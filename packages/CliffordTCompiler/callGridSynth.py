...
def decimalGridSynth(angle, precision):
    if (angle < 0): #correct formatting for gridsynth with negative angle
        angle = '"('+str(angle)+')"'
    cmd = f"./packages/CliffordTCompiler/gridsynth "+str(angle)+" -d "+str(precision); #system command formatted for linux, ensure oath to gridsynth is correct 
...
def binaryGridSynth(angle, precision):
    if (angle < 0): #correct formatting for gridsynth with negative angle
        angle = '"('+str(angle)+')"'
    cmd = f"./packages/CliffordTCompiler/gridsynth "+str(angle)+" -b "+str(precision); #system command formatted for linux, ensure oath to gridsynth is correct
    command_output = subprocess.check_output(cmd, shell=True).decode('utf-8')
    command_output = command_output[:-1] # Drop the trailing newline
    sequence = command_output.replace('W','')# drop the global phase
...
def epsilonGridSynth(angle, precision):
    ...
    cmd = f"./packages/CliffordTCompiler/gridsynth "+str(angle)+" -e "+str(precision); #system command formatted for linux, ensure oath to gridsynth is correct
    command_output = subprocess.check_output(cmd, shell=True).decode('utf-8')
    command_output = command_output[:-1] # Drop the trailing newline
    sequence = command_output.replace('W','')# drop the global phase
    sequence = sequence[::-1] # reverse to go from operaor to circuit form
